...
public abstract class XYChart extends AbstractChart {
        ...
        if (maxScaleNumber2 >= 0) {
            ...
            for (int i6 = 0; i6 < sLength; i6++) {
                ...
                if (series2.getItemCount() != 0) {
                    ...
                    synchronized (series2) {
                        int startIndex = -1;
                        for (Entry<Double, Double> value : series2.getRange(minX[scale2], maxX[scale2], seriesRenderer.isDisplayBoundingPoints()).entrySet()) {
                            ...
                            arrayList2.add((Double) value.getKey());
                            arrayList2.add((Double) value.getValue());
            ...
            if (showLabels || showGridX) {
                ...
                if (showLabels) {
                    ...
                    for (int i7 = 0; i7 < maxScaleNumber2; i7++) {
                        Align axisAlign = this.mRenderer.getYAxisAlign(i7);
                        for (Double location : this.mRenderer.getYTextLabelLocations(i7)) {
                            if (minY[i7] <= location.doubleValue() && location.doubleValue() <= maxY[i7]) {
                                ...
                                if (or == Orientation.HORIZONTAL) {
                                    if (axisAlign == Align.LEFT) {
                                        ...
                                        drawText(canvas, label, (float) left, yLabel - 2.0f, paint, this.mRenderer.getYLabelsAngle());
                                    } else {
                                        canvas.drawLine((float) right, yLabel, (float) (getLabelLinePos(axisAlign) + right), yLabel, paint);
                                        drawText(canvas, label, (float) right, yLabel - 2.0f, paint, this.mRenderer.getYLabelsAngle());
                                    }
                                    ...
                                    drawText(canvas, label, (float) (right + 10), yLabel - 2.0f, paint, this.mRenderer.getYLabelsAngle());
                                    if (showCustomTextGrid) {
                                        paint.setColor(this.mRenderer.getGridColor());
                ...
                if (showLabels) {
                    ...
                    if (or == Orientation.HORIZONTAL) {
                        ...
                        for (int i8 = 0; i8 < maxScaleNumber2; i8++) {
                            if (this.mRenderer.getYAxisAlign(i8) == Align.LEFT) {
            ...
            if (this.mRenderer.isShowAxes()) {
                ...
                for (int i9 = 0; i9 < maxScaleNumber2 && !rightAxis; i9++) {
                    rightAxis = this.mRenderer.getYAxisAlign(i9) == Align.RIGHT;
                }
                if (or == Orientation.HORIZONTAL) {
    ...
    /* access modifiers changed from: protected */
    public void drawSeries(XYSeries series, Canvas canvas, Paint paint, List<Float> pointsList, SimpleSeriesRenderer seriesRenderer, float yAxisValue, int seriesIndex, Orientation or, int startIndex) {
    ...
    public void drawChartValuesText(Canvas canvas, XYSeries series, SimpleSeriesRenderer renderer, Paint paint, List<Float> points, int seriesIndex, int startIndex) {
        ...
        for (int k2 = 0; k2 < points.size(); k2 += 2) {
            drawText(canvas, getLabel(renderer.getChartValuesFormat(), series.getY((k2 / 2) + startIndex)), ((Float) points.get(k2)).floatValue(), ((Float) points.get(k2 + 1)).floatValue() - renderer.getChartValuesSpacing(), paint, 0.0f);
        }
    }
            ...
            for (int j = 0; j < length; j++) {
                ...
                if (or == Orientation.HORIZONTAL) {
                    ...
                    if (showLabels && !textLabel) {
                        paint.setColor(this.mRenderer.getYLabelsColor(i));
                        canvas.drawLine((float) (right - getLabelLinePos(axisAlign)), yLabel, (float) right, yLabel, paint);
                        drawText(canvas, getLabel(this.mRenderer.getLabelFormat(), label), ((float) (right + 10)) + this.mRenderer.getYLabelsPadding(), yLabel - 2.0f, paint, this.mRenderer.getYLabelsAngle());
                    }
                    if (showGridX) {
                        paint.setColor(this.mRenderer.getGridColor());
                        canvas.drawLine((float) right, yLabel, (float) left, yLabel, paint);
