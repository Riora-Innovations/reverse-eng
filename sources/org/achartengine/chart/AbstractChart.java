...
public abstract class AbstractChart implements Serializable {
    ...
    public void drawString(Canvas canvas, String text, float x, float y, Paint paint) {
        if (text != null) {
            String[] lines = text.split(CSVWriter.DEFAULT_LINE_END);
            Rect rect = new Rect();
            int yOff = 0;
    ...
    public String getLabel(NumberFormat format, double label) {
        ...
        if (label == ((double) Math.round(label))) {
            return new StringBuilder(String.valueOf(Math.round(label))).toString();
        }
        return new StringBuilder(String.valueOf(label)).toString();
    }
        ...
        if (points.size() >= 4) {
            ...
            for (int i = 4; i < length; i += 2) {
                if ((((Float) points.get(i - 1)).floatValue() >= 0.0f || ((Float) points.get(i + 1)).floatValue() >= 0.0f) && (((Float) points.get(i - 1)).floatValue() <= ((float) height) || ((Float) points.get(i + 1)).floatValue() <= ((float) height))) {
                    float[] tempDrawPoints2 = calculateDrawPoints(((Float) points.get(i - 2)).floatValue(), ((Float) points.get(i - 1)).floatValue(), ((Float) points.get(i)).floatValue(), ((Float) points.get(i + 1)).floatValue(), height, width);
                    if (!circular) {
