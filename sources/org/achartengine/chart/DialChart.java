...
public class DialChart extends RoundChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        for (int i3 = 0; i3 < count2; i3++) {
            ...
            drawNeedle(canvas, angle, this.mCenterX, this.mCenterY, (double) shortRadius, this.mRenderer.getVisualTypeForIndex(i3) == Type.ARROW, paint);
        }
    ...
    private void drawTicks(Canvas canvas, double min, double max, double minAngle, double maxAngle, int centerX, int centerY, double longRadius, double shortRadius, double ticks, Paint paint, boolean labels) {
        ...
        while (i <= max) {
            ...
            if (labels) {
                ...
                String text = new StringBuilder(String.valueOf(i)).toString();
                if (Math.round(i) == ((long) i)) {
                    text = new StringBuilder(String.valueOf((long) i)).toString();
                }
