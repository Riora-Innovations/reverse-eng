...
class ProcessInCtrl {
    ...
    /* access modifiers changed from: 0000 */
    public DriverParameters getParams() {
        return this.mParams;
    }

    /* access modifiers changed from: 0000 */
    public InBuffer getBuffer(int idx) {
        ...
        if (bufSize > 0) {
            ...
            for (int i = 0; i < nrPackets; i++) {
                if (i == nrPackets - 1) {
                    lim = bufSize;
                    buffer.limit(lim);
                    int pos2 = i * this.mMaxPacketSize;
    ...
    public int processEventChars(boolean fRxChar, short sEvents, short slEvents) throws InterruptedException {
        ...
        if (!(signalEvents == 0 || (Events.Mask & 2) == 0 || (this.mDevice.mEventMask ^ 2) != 2)) {
        ...
        if (!(signalLineEvents == 0 || (Events.Mask & 4) == 0 || (this.mDevice.mEventMask ^ 4) != 4)) {
